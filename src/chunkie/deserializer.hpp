...
namespace chunkie {
template <typename HeaderType = uint32_t> class deserializer {
public:
...
public:
  ...
  void set_buffer(const uint8_t *data, header_type size) {
    ...
    m_buffer_reader =
        std::make_unique<endian::stream_reader<endian::big_endian>>(data, size);

    read_header();
  }

  /// @returns true if all data in the set buffer have been processed
  bool buffer_proccessed() const { return m_buffer_reader == nullptr; }
  ...
  bool object_completed() const { return m_object_completed; }
...
template <class T> const T deserializer<T>::header_size = sizeof(T);
} // namespace chunkie
