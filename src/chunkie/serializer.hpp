...
/// the added overhead.
template <typename HeaderType = uint32_t> class serializer {
public:
...
public:
  ...
  /// to a buffer
  bool object_proccessed() const { return m_object == nullptr; }
  ...
  void write_buffer(uint8_t *data, header_type size) {
    ...
    auto bytes = std::min<header_type>(size - header_size, m_object_remaining);
...
template <class T> const T serializer<T>::header_size = sizeof(T);
} // namespace chunkie
